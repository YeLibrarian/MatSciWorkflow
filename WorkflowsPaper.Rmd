...
abstract: "While small labs produce much of the fundamental experimental research
  in Material Science and Engineering (MSE), little is known about their data management and sharing practices and the extent to which they promote trust in, and transparency of, the
  published research.\n In this research, we conduct a case study of a leading MSE
  research lab to characterize the limits of current data management and sharing practice concerning
  reproducibility and attribution. We systematically reconstruct the
  ...
  or at risk. \n We find that while data management and sharing practices in this leading lab protect
...
doc_is_html <- (opts_knit$get("rmarkdown.pandoc.to")=="html")
...
assess reproducibility and improve scientific practice systematically [@ReproducibilityReplicabilityScience2019]. More recently, there have been high-profile calls and initiatives by research societies, funders, and publishers to make
...
[@plante2021]; a proposed controlled vocabulary and metadata schema for materials discovery [@medina-smith2021]; and a
...
Astronomy e.V.), a European-originated effort, aims to build a reliable infrastructure for data from materials
...
infrastructure requirements and demonstrate a re-imagined role of humans in ensuring data is appropriately managed,
...
specifically, are successful in obtaining external research support. Professor Jaramillo is interested in
reproducible research and open science: he has published in this area, his group has developed related software prototypes
...
continuous monitoring of lab equipment. This collaboration is facilitated by shared information repositories, including a
...
type of action, equipment, and methods used were recorded using standardized codes. (See *Appendix II* for the coding method and dictionary.) This tabular data was then used to impute collaboration and data-dependency graphs (see the *Results* section below).
...
flow graph). (See Appendix III-A for details)



## Digital Forensics

```{r file-forensics-analysis,  echo=doc_is_html}
...
```{r forensics-timeseries,  echo=doc_is_html}
...
with the timeline of the projects we evaluated, we restricted our analysis to files deposited between January 2019
and January 2022.

Overall, deposit patterns demonstrated that the repository was actively used -- with deposit rates varying seasonally.
(See *Appendix V*). All information was collected for `r nrow(recent_files.tbl)` files -- including file names, paths,
...
See *Appendix III-B* for details on post-analysis validation through follow-up interviews and documentation review. 
...
```{r wkflow-graph-func, echo=doc_is_html, include=FALSE}
...
```{r workflow-a,  echo=doc_is_html, fig.width=7,fig.height=7}
...
used for recording measurements on lab tools that are shared to maintain a consistent tool log (required by the
Professor). The group Dropbox is used for saving raw data directly from instruments. Post-processed data is saved to a
personal Dropbox.

```{r workflow-b,  echo=doc_is_html, fig.width=7,fig.height=7}
...
characterize it, and analyze the results. Finally, they transform the sample via a laser set-up; this process is
...
```{r workflow-c,  echo=doc_is_html, fig.width=7,fig.height=7}
...
```{r workflow-d,  echo=doc_is_html, fig.width=7,fig.height=7}
...
work can be represented as a set of independent, self-contained tasks. (Summary graph statistics are shown in
*Appendix V, Table A1*.) Most of the tasks contain only one atomic action. Further, there is a rhythm across each workflow
...
```{r  descriptives, echo=doc_is_html, fig.height=7,fig.width=7}
...
Figure 3 provides an answer to the first [research question](#ResearchQuestions), concerning automation:
...
transmission of information (e.g., for instrument setup or contextualization of the analysis); and a substantial
incidence of e-mail and portable media for information storage. Together, this suggests that there is a significant
...
```{r graph-statistics, echo=doc_is_html, include=FALSE}
...
The information connections within and across projects (Figure 4) are densely interconnected within projects -- a
...
```{r  flow-graphs-info,  echo=doc_is_html}
# information network
...
Collaboration networks (Figure 5) are also partitioned by project/workflow. The size of the grid varies substantially
across projects.

```{r flow-graphs-collab,  echo=doc_is_html,   warning=FALSE}
collaborator.tg %>%
  activate(nodes) %>%
...
During the interviews, one interviewee, who self-identified as a founding member of the group, mentioned a document titled “Jaramillo group new member checklist,” which described shared computing resources, lab safety, and access, as well as data management practices. We reviewed the document and the group Wiki site to summarize documented practices. We also compared the documented practice with practices elicited through interviews for relevance to our research questions, and to inform the interpretation of the workflow networks below.

We identified the practices most relevant to data management and scientific workflow management and grouped them into
three categories -- information sharing, information security, and information organization. (See *Appendix IV*.)
...
```{r data-curation-features,  echo=doc_is_html}
...
```{r collaboration-features,  echo=doc_is_html}
...
To address the remaining research questions, we measured and compared the mathematical graphs describing the workflow
...
```{r outputs-table,   echo=doc_is_html}
    outputs.tbl %>%
...
Note that the existence of documentation is necessary for unassisted replication is not sufficient: We did not
...
The following [research question](#ResearchQuestions) concerns the robustness of storage practices:
...
graph to trace the flow of these objects across tasks and into storage locations. From this set of traces, we can infer
the content of the designated group storage location post-analysis. The results are summarized in the table below.

```{r storage-table,  echo=doc_is_html}
...
outputs.tbl %>% 
  ...
  arrange(managed,object_role) %>%
      ...
      tab_header("Table 3: Proportion of output in managed storage, by type.", subtitle = "A substantial portion of highlighted outputs are at risk.") %>%
...
delay is relatively small (40% of these files were deposited within `r REP_v_plag50days` day). However, a substantial
...
because of the inconsistent use of documented naming practices and the variation of undocumented practices.)
...
```{r collaboration-table,  echo=doc_is_html}
...
-   Recommendation 3: All of the reported workflows involved the extensive use of personal portable storage to transfer
    ...
    that manual data transfer and operations will increase errors.
...
-   Recommendation 5: Develop explicit practices around collaborator attribution. Practices are needed to identify the
    ...
    documentation (e.g., in the lab notebook) to identify when the researcher uses externally contributed resources, borrowed equipment, or information received from a collaborator, (b) explicitly saving contributed data,
...
-   Recommendation 6: Develop explicit practices around reproducibility beyond the stage of raw data.
    ...
    -   Make documentation for standard practices at commonly used equipment in external locations (i.e., other MIT
        facilities, like MRL and DMSE). Consistent practices at these facilities would allow for the comprehensible
...
In this article, we have identified gaps in an exemplar set of materials science workflow processes and characterized
...
professional training [@altman2018] -- research is needed on how effective practices can be aligned with incentives,
...
the barriers to starting new practices. Improvement of interfaces for human-computer interaction, accessibility, and
...
The interview protocol used in the study is included as file *Appendix_I_InterviewProtocol.md*

## Appendix II. Coding Dictionary
...
The coding dictionary used in the study is included as file *Appendix_II_CodeDictionary_deidentified.ods*

## Appendix III. Data Analysis and Post Validation Details

### Appendix III-A. Process of Creating Workflow Graphs

The process of creating the workflow graph is summarized below. (For replication purposes, we have placed all of the
...
We conducted semi-structured follow-up interviews with all participants to assess the strength of (dis)agreement with the analysis
described above and its main conclusions, and with the recommendations below; and to probe for additional comments,
...
## Appendix V. Tables and Figures

```{r supplementary-tables, echo=doc_is_html}
ta1.gt
ta2.gt
